...
namespace mb
{
	...
	private:
		...
		mutable std::unique_ptr<http_service> _httpService;
	...
	public:
		...
		constexpr std::string_view id() const noexcept override { return exchange_ids::DEXTRADE; }

		std::weak_ptr<websocket_stream> get_websocket_stream() override 
		{ 
			throw not_implemented_exception{ "dextrade::get_websocket_stream" };
		}
